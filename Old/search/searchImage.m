...
%                  (the classifier confidence).
	...
	for i = 1 : length(scaleRange)
		...
		% image.
		[allHistogramsL, xoffset, yoffset] = getHistogramsForImage(hog, img(:,:,1));
        [allHistogramsA, xoffset, yoffset] = getHistogramsForImage(hog, img(:,:,2));
        [allHistogramsB, xoffset, yoffset] = getHistogramsForImage(hog, img(:,:,3));
        
		cellRow = 1;
		
		% For each row of the image...
		while((cellRow + hog.numVertCells - 1) <= size(allHistogramsL, 1))
			...
			while ((cellCol + hog.numHorizCells - 1) <= size(allHistogramsL, 2))
				...
				histogramsL = allHistogramsL(cellRowRange, cellColRange, :);
                histogramsA = allHistogramsA(cellRowRange, cellColRange, :);
                histogramsB = allHistogramsB(cellRowRange, cellColRange, :);
                
				% Compute the HOG descriptor.
				HL = getDescriptorFromHistograms(hog, histogramsL);            
				HA = getDescriptorFromHistograms(hog, histogramsA);
				HB = getDescriptorFromHistograms(hog, histogramsB);
