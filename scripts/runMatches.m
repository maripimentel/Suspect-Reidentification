...
fprintf('Making the test for 1 suspect in 1 images.\n');
...
% Read the original image.
    ...
    imgOriginal = imread('./Matches/Suspect1/crop001504b.png');
    ...
    clear bboxes2;
        ...
        img = imread('./Matches/Suspect1/person_272.png');
...
%         img = imgSave;
        ...
        peopleDetector128x64 = vision.PeopleDetector('ClassificationModel','UprightPeople_128x64', 'ClassificationThreshold', 3, 'MergeDetections', true, 'WindowStride', [4,4]);
        [bboxes1, scores1] = peopleDetector128x64(img);
        peopleDetector96x48 = vision.PeopleDetector('ClassificationModel','UprightPeople_96x48', 'ClassificationThreshold', .5, 'MergeDetections', true, 'WindowStride', [4,4]);
...
%         export_fig(sprintf('./Test/Suspect%d/DiscreteWeight/ImagesWithSuspect/test%d.png', contSusp, cont), '-native');
        ...
        figure;
        imagesc(img);
        title('Reidentification top 1 (Average)');
        hold on;
        plot(0,0,'r');
%         plot(0,0,'g');
% 
...
%         end
        ...
        addpath('./export_fig/');
        export_fig(sprintf('./Matches/Suspect1/averageResult.png'), '-native');


        %Mode Technique
        figure;
        imagesc(img);
        title('Reidentification top 1 (Mode)');
        hold on;
        plot(0,0,'r');
%         plot(0,0,'g');
% 
...
%         end
        ...
        addpath('./export_fig/');
        export_fig(sprintf('./Matches/Suspect1/modeResult.png'), '-native');
       
        %Discrete Weight Technique
        figure;
        imagesc(img);
        title('Reidentification top 1 (DiscreteWeight)');
        hold on;
        plot(0,0,'r');
%         plot(0,0,'g');
% 
...
%         end
        ...
        addpath('./export_fig/');
        export_fig(sprintf('./Matches/Suspect1/discreteResult.png'), '-native');
